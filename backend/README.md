...
## 6. コードの更新方法（URLを変えずに更新）
コードを修正・上書きした際、既存のURL（固定URL）のまま反映させるには以下の手順を行ってください。

1. 右上の **「デプロイ」** > **「デプロイを管理」** をクリックします。
2. 左側のリストから、現在使用中のデプロイ（ウェブアプリ）を選択し、右上の **「編集（鉛筆アイコン）」** をクリックします。
3. バージョンのドロップダウンで **「新バージョン」** を選択します。
4. 右下の **「デプロイ」** ボタンをクリックします。

※「新しいデプロイ」を毎回選ぶとURLが変わってしまうため注意してください。
